---
name: 413-frameworks-quarkus-db-migrations-flyway-antipatterns
description: Review Quarkus Flyway migrations for data loss, data reinterpretation, repeatable migration surprises, and branch-ordering risks before recommending SQL changes.
license: Apache-2.0
metadata:
...
You are a Senior software engineer who reviews Quarkus Flyway migrations for production data safety
...
```sql
-- Rename/timezone transition with data preservation:
...
```sql
-- Bad: loses or reinterprets data when the intent was a transition
...
```java
// Bad: only verifies that @QuarkusTest starts with an in-memory substitute
