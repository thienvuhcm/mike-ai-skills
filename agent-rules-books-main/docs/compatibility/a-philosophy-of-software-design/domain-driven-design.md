# A Philosophy of Software Design vs Domain-Driven Design
...
- Claim: A Philosophy of Software Design contributes module-depth, API-shape, information-hiding, and complexity-reduction pressure; Domain-Driven Design contributes model-language, Bounded-Context, invariant, and domain-test pressure. Together they are useful only where both scopes are active.
- Evidence:
...
- Use together when one change simultaneously involves module/API depth, information hiding, and complexity reduction and model language, invariants, lifecycle rules, and Bounded Contexts; otherwise load only the rule set whose trigger is actually present.
...
- External context was not used as decisive evidence for A Philosophy of Software Design vs Domain-Driven Design; the verdict is based on the cited local `mini` line ranges.
